    ...
    public static void start(Class<TestSet> c) {
        ...
        for (Method m : arrMethods) {
            if (m.isAnnotationPresent(Test.class)) {
                ...
                if (bsMethod != null) {
                    throw new RuntimeException("Можно использовать только одну аннотацию @BeforeSuite");
                }
                bsMethod = m;
            } else if (m.isAnnotationPresent(AfterSuite.class)) {
                if (asMethod != null) {
                    throw new RuntimeException("Можно использовать только одну аннотацию @AfterSuite");
                }
                asMethod = m;
            }
        }
